...
export default async function CollectionAnalyticsPage() {
  ...
  const changePct =
    prevValueCents > 0
      ? (changeCents / prevValueCents) * 100
      : null;
      ...
      <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-white/80">
            Track your portfolio value, daily changes, and where your
        ...
        <div className="rounded-2xl border border-white/15 bg-white/5 p-4 backdrop-blur-sm">
          ...
          {changeCents === 0 && daily.length > 1 && (
            <div className="mt-2 text-sm text-white/60">
              No change vs previous snapshot
            </div>
          )}
          {daily.length <= 1 && (
            <div className="mt-2 text-sm text-white/60">
        ...
        <div className="rounded-2xl border border-white/15 bg-white/5 p-4 backdrop-blur-sm space-y-2">
          ...
          <div className="flex flex-col gap-1 text-sm">
            ...
            {unrealizedCents != null && costCents && costCents > 0 && (
              <div className="flex items-center justify-between">
                ...
                <span className="font-medium">
                  {((unrealizedCents / costCents) * 100).toFixed(1)}%
                </span>
              </div>
            )}
          </div>
          {latest && (
            <div className="mt-2 text-xs text-white/60">
              Qty: {latest.total_quantity} • Items:{" "}
              {latest.distinct_items}
            </div>
          )}
        </div>
      ...
      <div className="rounded-2xl border border-white/15 bg-white/5 p-4 backdrop-blur-sm">
        ...
        {history.length === 0 ? (
          <div className="rounded-md border border-white/10 bg-white/5 p-3 text-sm text-white/80">
            No history yet. Keep your nightly revalue job running and
          ...
          <div className="mt-3 h-40 w-full">
            <svg
              viewBox="0 0 260 120"
              className="h-full w-full text-emerald-300"
            >
              {/* baseline */}
              <line
                x1="0"
      ...
      <div className="rounded-2xl border border-white/15 bg-white/5 p-4 backdrop-blur-sm">
        ...
        {recent.length === 0 ? (
          ...
          <ul className="divide-y divide-white/10">
            {recent.map((r) => {
              const valueCents =
                r.last_value_cents != null
                  ? r.last_value_cents * r.quantity
                  : null;
