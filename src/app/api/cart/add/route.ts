...
async function getOrCreateCartId(): Promise<string> {
  ...
  // Create cart (id should be generated by DB default)
...
export async function POST(req: Request) {
  try {
    ...
    const quantity = toQty(body?.quantity ?? 1);
    ...
    await db
      .insert(cart_lines)
      .values({
        cart_id: cartId,       // ✅ variable -> column
        listing_id: productId, // ✅ snake_case
        qty: quantity,
      })
      .onConflictDoUpdate({
        target: [cart_lines.cart_id, cart_lines.listing_id],
        set: {
          qty: sql`${cart_lines.qty} + ${quantity}`, // ✅ cart_lines, not cartLines
          updated_at: sql`now()`,                    // ✅ snake_case
        },
      });
