...
function makeFunkoId(idRaw: string | null, upcRaw: string | null): string | null {
...
// Accepts: "14.99", "$14.99", "1499", "1,499", " 14.99 "
// Heuristic:
// - if contains "." => dollars
// - else if >= 1000 => cents (1499)
// - else => dollars (15 => $15.00)
...
export async function upsertFunkoFromForm(formData: FormData) {
  ...
  try {
    ...
    const id = makeFunkoId(idRaw, upc);
    if (!id) {
      redirect(
    ...
    // Optional: also create/update a Product listing
    const alsoCreateProduct = b(formData.get("also_create_product"));

    // Product fields — we FORCE funko/single server-side
    ...
    if ((submittedGame && submittedGame !== "funko") || (submittedFormat && submittedFormat !== "single")) {
      redirect(
        `${backTo}?error=${encodeURIComponent(
          `Invalid product_game/product_format submitted. Funko must be game=funko format=single.`,
        )}`,
      );
    }

    const productGame: "funko" = "funko";
    ...
    // Weight from extra if present (numeric(6,2))
    ...
    const result = await db.transaction(async (tx) => {
      ...
      // 2) Resync funko_item_images deterministically
      await tx.execute(sql`
        DELETE FROM public.funko_item_images
        WHERE item_id = ${id}
      `);
      ...
      // 3) Upsert products
      const baseTitle = name ?? `Funko Item ${id}`;
      ...
      const baseSlug = `funko-${slugify(baseTitle)}-${slugify(numPart)}`.replace(/-+/g, "-");
      const productSlug = baseSlug || `funko-${slugify(id)}`;
      ...
      const skuOkRows = await tx.execute<{ ok: boolean }>(sql`
        SELECT NOT EXISTS(
          ...
          WHERE p.sku = ${skuCandidate}
            AND (p.funko_item_id IS DISTINCT FROM ${id})
        ) AS ok
      `);
      ...
      const upsertProductRows = await tx.execute<{ id: string }>(sql`
        ...
        ON CONFLICT (slug) DO UPDATE SET
          ...
          funko_item_id = EXCLUDED.funko_item_id,
      ...
      // 4) Resync product_images from funko_item_images
      await tx.execute(sql`
        DELETE FROM public.product_images
        WHERE product_id = ${productId}::uuid
      `);

      const funkoImgs = await tx.execute<{ sort_order: number; label: string | null; url: string }>(sql`
      ...
      const rows =
        ((funkoImgs as any)?.rows ?? []) as Array<{ sort_order: number; label: string | null; url: string }>;
      ...
      // 5) Sync tags into tags + product_tags
      if (extraTags.length) {
        const tagIds: string[] = [];
    ...
    redirect(`/categories/funko/items/${encodeURIComponent(id)}?ok=${encodeURIComponent(okMsg)}`);
  } catch (e: any) {
