...
async function main() {
  ...
  try {
    console.log("=== upgradePlaceholderImagesFromRawTables.v5_1: start ===");

    // Pull placeholder images
    // NOTE: your product_images has no "position", so we order by created_at if present, else id.
    const { rows } = await client.query(`
      ...
      ORDER BY p.game, p.format, p.title, pi.id
    `);
